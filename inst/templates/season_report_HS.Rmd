...
datatable(
  nodata(
    data = get(names(testlist[3])), 
    ref_data = get(names(testlist[1])), 
    species = "all"), 
...
knitr::kable(
  submitted(get(names(testlist[3]))) %>% 
...
submitted(get(names(testlist[3])), type = "state", output = "plot") +
...
knitr::kable(
  submitted(get(names(testlist[2]))) %>% 
    rename(`Has submitted?` = has_submitted) %>% 
    mutate(prop = round(n/sum(n), 2)))
```
...
submitted(get(names(testlist[2])), type = "state", output = "plot") +
  scale_fill_manual(values = met.brewer("Hokusai3", 2)) + 
  theme(legend.position = "bottom")
```
...
hunted(get(names(testlist[3])), type = "state", output = "plot") +
  scale_fill_manual(values = met.brewer("Hokusai3", 2)) + 
  theme(legend.position = "bottom")
```
...
hunted(get(names(testlist[2])), type = "state", output = "plot") +
  scale_fill_manual(values = met.brewer("Hokusai3", 2)) + 
  theme(legend.position = "bottom")
```
...
hunted(get(names(testlist[2])), type = "species", output = "plot") +
  scale_fill_manual(values = met.brewer("Hokusai3", 2)) + 
  theme(legend.position = "bottom")
```
...
datatable(
  partyhunt(get(names(testlist[2]))) %>% 
    select(-unretrieved), 
...
datatable(
  overdays(get(names(testlist[3])), get(names(testlist[1])))
)
```
...
datatable(
  overdays(get(names(testlist[2])), get(names(testlist[1]))) 
)
```
...
datatable(
  overbags(get(names(testlist[3])), get(names(testlist[1]))) 
)
```
...
overbags(get(names(testlist[2])), get(names(testlist[1]))) %>% 
  rename(
    state = sampled_state,
...
datatable(
  overbags(get(names(testlist[2])), get(names(testlist[1])), summary = T) %>% 
    rename(
      state = sampled_state,
      sp = sp_group_estimated) %>% 
    mutate(
...
datatable(
  overbags(get(names(testlist[2])), get(names(testlist[1])), summary = F) %>% 
    rename(
      state = sampled_state,
      sp = sp_group_estimated) %>% 
    mutate(
    ...
    mutate(over = "Y") %>% 
    bind_rows(
      overbags(get(names(testlist[2])), get(names(testlist[1])), over = F) %>% 
        rename(
          sp = sp_group_estimated,
          state = sampled_state) %>% 
        mutate(
...
openclose(get(names(testlist[2])), get(names(testlist[1])), summary = T) %>% 
  rename(
    sp = sp_group_estimated,
    state = sampled_state) %>% 
  left_join(
...
datatable(openclose(get(names(testlist[2])), get(names(testlist[1])), summary = T))
```
...
retrievedmap(get(names(testlist[2])))
...
retrieved(get(names(testlist[2])), output = "plot", average = T) +
  theme(legend.position = "bottom") +
  scale_fill_manual(
    values = 
      met.brewer("Hokusai3", 
                 length(unique(get(names(testlist[2]))$sp_group_estimated))))

datatable(retrieved(get(names(testlist[2])), average = T))
```
...
bagspp(get(names(testlist[2])), output = "popular")
```
...
huntdays(get(names(testlist[2])), type = "species", output = "plot", average = T)
```
...
huntdays(get(names(testlist[2])), output = "plot", average = T) +
  theme(legend.position = "bottom") + 
  scale_fill_manual(
...
datatable(huntdays(get(names(testlist[2])), average = T))
```
...
compare(get(names(testlist[2])), get(names(testlist[3])), type = "line")
```
...
compare(get(names(testlist[2])), get(names(testlist[3])), type = "days")
```
...
compare(get(names(testlist[2])), get(names(testlist[3])), type = "retrieved")
```
