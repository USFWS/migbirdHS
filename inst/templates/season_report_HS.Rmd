...
How many records were submitted in the season data? Data are only shown if days_hunted > 0.
...
How many records were finalized in the daily data? Records must have answered "yes" to has hunted.
...
errorsHS <-
  function(data){
    data %>% 
      select(errors) %>% 
      filter(errors != "none") %>% 
      mutate(errors = ifelse(str_detect(errors, "-"), "overbag and overday", errors)) %>% 
      group_by(errors) %>% 
      summarize(n = n()) %>% 
      ungroup() %>% 
