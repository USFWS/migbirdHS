...
Are there any party hunts in the daily data that we can identify via comments?
...
proofed_parties <-
  read_csv(paste0(params$partypath, "proofed_parties_", params$year, ".csv"))

datatable(proofed_parties)
```
...
```{r, proofandcorrect, message = FALSE, warning = FALSE, echo = F}
...
daily_proofed <- proofHS(party_proofed)
rm(party_proofed)
daily_corrected <- correctHS(daily_proofed)
rm(daily_proofed)

# Proof and correct the season data
season_proofed <- proofHS(get(names(tibblelist[3])))
...
```{r, errors_season, message = FALSE, warning = FALSE, echo = F}
...
errorsHS <-
  function(data){
    data %>% 
      ...
      geom_text(aes(x = errors, y = prop, label = n), nudge_y = .1) +
      theme_classic()
  }
...
```{r, errors_daily, message = FALSE, warning = FALSE, echo = F}
...
bagspp(daily_corrected, output = "popular")
```
...
What is the relationship between number of birds retrieved and number of days spent hunting when data are divided into 4 submission groups? The corrected daily and season data are used in the figures below.
